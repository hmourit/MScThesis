...
def load_and_pickle_mdd_data():
    ...
    with open(pickled_data_folder + mdd_rma_pickled_file, 'wb') as out:
        pickle.dump(rma, out)
    ...
    with open(pickled_data_folder + mdd_target_pickled_file, 'wb') as out:
...
def load_mdd_data():
    if not os.path.isfile(pickled_data_folder + mdd_rma_pickled_file) \
       or not os.path.isfile(pickled_data_folder + mdd_target_pickled_file):
        print('Pickled data not found, loading and pickling...')
        data, target = load_and_pickle_mdd_data()
    else:
        with open(pickled_data_folder + mdd_rma_pickled_file, 'rb') as in_:
            data = pickle.load(in_)
        with open(pickled_data_folder + mdd_target_pickled_file, 'rb') as in_:
